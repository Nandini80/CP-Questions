...
void solve() 
{
  ...
  for(int i=0 ; i<n ;i ++){
    ...
    else before0[i] = before0[i-1>0?i-1:0];
  }   

  for(int i=n-2 ; i>=0 ; i--){
    if(s[i+1]=='1') after1[i] = after1[i+1]+1;
    else after1[i] = after1[i+1];
  }

  vl sum(n);
  for(int i=0 ; i<n ; i++){
    sum[i] = before0[i]+after1[i];
    // cout << sum[i] << " ";
  }
  // cout << endl;
  ...
  mini[n-1]= sum[n-1];
  for(int i=n-2 ; i>=0 ;i--){
    mini[i] = min(sum[i],mini[i+1]);
  }
  // hum haar ind ko start manege aur min sum vali value tk 1's consider karege 
  // aur yeh krne ki cost nikalege jiski min vo ans

  ll ones=0,zeros=0,cost=INT_MAX; // uss ind se phele kitne ones and zeros aye
  for(int i=0 ; i<n ; i++){
    cost = min(cost,mini[i]+ones-zeros);
    if(s[i]=='1') ones++;
    else zeros++;
  }
  pe(cost);
