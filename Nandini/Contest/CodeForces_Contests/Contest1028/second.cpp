...
{
   ll n; cin >> n;
   vl p(n),q(n); ipt(p,n); ipt(q,n);

   int ind1 = 0,ind2=0,maxi= max(p[0],q[0]);

   for(int i=0 ; i<n ; i++){
     ll val;
     if(p[ind1]==q[ind2]){
        if(q[i-ind1]>p[i-ind2]){
            val =  (binexp(2,p[ind1])+binexp(2,q[i-ind1]))%mod;
        }
        else
        val = (binexp(2,p[i-ind2])+binexp(2,q[ind2]))%mod;
     }
     else {
        if(p[ind1]==maxi){
           val = (binexp(2,p[ind1])+binexp(2,q[i-ind1])%mod)%mod;
        }
        else {
           val = (binexp(2,p[i-ind2])%mod+binexp(2,q[ind2])%mod)%mod;
        }
     }
     cout << val%mod << " ";
