...
int fibbo(vector<ll> &v,int ind){
    if(ind==1 || ind==2 || ind==0) return ind;
    if(v[ind-1]!=-1) return v[ind-1];
    return v[ind-1] = fibbo(v,ind-1)+fibbo(v,ind-2);
}
void solve()  
{
   ll n,m; cin >> n >> m;
   ...
   int ind = n-1;
   for(auto it : dim){
    int w = it[0], l = it[1], h = it[2];
    if(w<v[ind] || l<v[ind] || h<v[ind]) {
        ans+='0';
        continue;
    }
    ll needed = v[ind]+v[ind-1];
    if(w>=needed || l>=needed || h>=needed){
      ans+='1';
    }
    else ans+='0';
   }
